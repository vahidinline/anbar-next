import { ReactNode, ButtonHTMLAttributes, InputHTMLAttributes, TextareaHTMLAttributes, SelectHTMLAttributes, memo } from 'react';
import { cn } from '@/lib/utils';
import { X } from 'lucide-react';

type BtnVariant = 'primary' | 'secondary' | 'destructive' | 'outline' | 'ghost' | 'success';
export function Button({ variant = 'primary', className, ...p }: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: BtnVariant }) {
...
export function Modal({ open, onClose, title, children, footer }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="bg-card rounded-xl shadow-card w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between border-b px-5 py-3">
          <h3 className="font-semibold">{title}</h3>
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-muted"><X className="size-4" /></button>
        </div>
        <div className="p-5 overflow-y-auto flex-1">{children}</div>
        {footer && <div className="border-t px-5 py-3 flex justify-end gap-2 bg-muted/30">{footer}</div>}
      </div>
    </div>
  );
}

export function ConfirmDialog({ open, onClose, onConfirm, title = 'حذف', message, busy }: {
  open: boolean; onClose: () => void; onConfirm: () => void; title?: string; message: string; busy?: boolean;
}) {
  return (
    <Modal open={open} onClose={onClose} title={title} footer={
      <>
        <Button variant="secondary" onClick={onClose} disabled={busy}>انصراف</Button>
        <Button variant="destructive" onClick={onConfirm} disabled={busy}>تایید حذف</Button>
      </>
    }>
...
export function EmptyState({ message = 'موردی برای نمایش وجود ندارد', icon }: { message?: string; icon?: ReactNode }) {
  return (
...
function DataTableImpl({ columns, children, dir }: { columns: Col[]; children: ReactNode; dir?: 'ltr' | 'rtl' }) {
        ...
        <thead className="bg-muted/60 sticky top-0">
          <tr>
            {columns.map((c, i) => {
              ...
              return (
                <th key={`${label}-${i}`} className={cn('font-semibold px-3 py-2.5 whitespace-nowrap', alignClass(align))}>
                  {label}
                </th>
              );
            })}
...
export function Badge({ tone = 'default', children }: { tone?: 'default' | 'success' | 'destructive' | 'warning'; children: ReactNode }) {
  const tones = {
  ...
  return <span className={cn('inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium', tones[tone])}>{children}</span>;
}
