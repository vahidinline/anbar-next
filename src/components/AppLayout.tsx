...
import {
  LayoutDashboard, Settings, Layers, Package, Users, FileText, BarChart3,
  LogOut, Menu, X, Moon, Sun, Radar, ShieldCheck, History,
} from 'lucide-react';
...
export function AppLayout({ children }: { children: ReactNode }) {
  const { user, loading, signOut, has, roles, isActive } = useAuth();
      ...
      <aside className={cn(
        ...
        <div className="px-5 py-5 border-b border-sidebar-border flex items-center gap-3">
          <div className="size-10 rounded-lg bg-white border flex items-center justify-center p-1">
            <img src={logo} alt="فید ایران صنعت" className="max-h-full max-w-full object-contain" />
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-card border-b flex items-center justify-between px-4 lg:px-6 sticky top-0 z-20">
          ...
          <div className="flex items-center gap-2">
            <button onClick={toggleDark} className="p-2 rounded-md hover:bg-muted" title="حالت تیره/روشن">
        ...
        <main className="flex-1 p-4 lg:p-6 overflow-x-auto">
          {!isActive ? (
            <div className="max-w-md mx-auto mt-20 text-center bg-card border rounded-xl p-8">
              ...
              <p className="text-sm text-muted-foreground">برای فعال‌سازی با مدیر سیستم تماس بگیرید.</p>
            </div>
          ) : children}
        </main>
      </div>
