...
export function WarehouseCombobox({
  warehouses, value, onChange, placeholder = '— انتخاب انبار —', disabled = false,
}: {
  ...
  return (
    <Popover open={open} onOpenChange={(o) => !disabled && setOpen(o)}>
      ...
      <PopoverContent className="p-0 w-[--radix-popover-trigger-width] min-w-[260px]" align="start" dir="rtl">
        <Command shouldFilter={true} filter={(val, search) => (matchesSearch(val, search) ? 1 : 0)}>
          ...
          <CommandList className="max-h-72">
            ...
            <CommandGroup>
              {warehouses.map((w) => (
                <CommandItem key={w.id} value={w.name} onSelect={() => { onChange(w.id); setOpen(false); }}>
                  <Check className={cn('me-2 h-4 w-4', value === w.id ? 'opacity-100' : 'opacity-0')} />
