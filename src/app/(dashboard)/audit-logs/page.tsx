  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (q.trim()) {
        ...
        if (
          !matchesSearch(r.user_email || "", q) &&
          !matchesSearch(r.action, q) &&
          !matchesSearch(detailsStr, q)
        )
          return false;
      }
      return true;
  ...
  return (
    <>
      <PageHeader
        ...
        description="پیگیری تمامی عملیات انجام‌شده در سامانه"
      />
      <Card className="p-3 mb-4 flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <Input
            className="pr-9"
            placeholder="جستجو در کاربر، عملیات یا جزئیات..."
            value={q}
          ...
          {filtered.map((r) => (
            <tr key={r.id}>
              ...
              <td className="px-4 py-2.5 text-xs text-muted-foreground">
                {formatJalali(r.created_at.toISOString())} -{" "}
                {toFaDigits(
              ...
              <td
                className="px-4 py-2.5 text-xs text-muted-foreground max-w-xs truncate"
                title={r.details || ""}
              >
                {r.details || "—"}
              </td>
