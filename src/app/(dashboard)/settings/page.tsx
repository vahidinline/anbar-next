...
export default function SettingsPage() {
  ...
  const filtered = items.filter(
    (w) => matchesSearch(w.name, search) || matchesSearch(w.description || "", search),
  );
          ...
          {filtered.map((w, i) => (
            <tr key={w.id}>
              ...
              <td className="px-4 py-3 text-muted-foreground">
                {formatJalali(w.created_at.toISOString())}
              </td>
              <td className="px-4 py-3">
                <div className="flex gap-1">
                  <Button variant="ghost" onClick={() => openEdit(w)} className="px-2 py-1.5">
        ...
        <div className="space-y-4">
          <Field label="نام انبار" required>
            <Input
              ...
              placeholder="مثلاً: انبار سیاه سنگ"
            />
          </Field>
          <Field label="توضیحات">
            <Textarea
