...
import { Plus, Trash2, Search, FileText, ArrowDownToLine, ArrowUpFromLine, Printer, ListOrdered } from "lucide-react";
import { useSession } from "next-auth/react";
import { Button, Input, Field, Textarea, Select, Modal, ConfirmDialog, PageHeader, DataTable, EmptyState, Badge, Card } from "@/components/ui-kit";
...
import { fetchDocumentsData, fetchProductSerials, fetchDocumentSerials, saveDocument, deleteDocument } from "./actions";

interface ProductOpt { id: string; code: string; name: string; product_group_id: string | null }
function ProductCombobox({ products, groupMap, value, onChange }: { products: ProductOpt[]; groupMap: Record<string,string>; value: string; onChange: (id: string) => void }) {
  const [open, setOpen] = useState(false);
  const selected = products.find((p) => p.id === value);
  const selLabel = selected ? `${groupMap[selected.product_group_id ?? ''] ?? '—'} | ${selected.name}${selected.code ? ' | ' + selected.code : ''}` : '';
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button type="button" role="combobox" aria-expanded={open}
          ...
          <span className={cn('truncate', !selected && 'text-muted-foreground')}>{selected ? selLabel : '— انتخاب کالا —'}</span>
          <ChevronsUpDown className="h-4 w-4 opacity-50 shrink-0 ms-2" />
        </button>
      </PopoverTrigger>
      <PopoverContent className="p-0 w-[--radix-popover-trigger-width] min-w-[340px]" align="start" dir="rtl">
        <Command shouldFilter={true} filter={(val, search) => {
          ...
          <CommandList className="max-h-72">
            ...
            <CommandGroup>
              {products.map((p) => {
                ...
                return (
                  <CommandItem key={p.id} value={composite} onSelect={() => { onChange(p.id); setOpen(false); }}>
...
// Interfaces
interface Doc {
  id: string; doc_number: number; document_type: 'incoming' | 'outgoing';
  contact_id: string | null; product_id: string | null; product_group_id: string | null;
  warehouse_id: string | null;
  quantity: number; unit: string | null; document_date: string; description: string | null;
}
interface Product { id: string; code: string; name: string; product_group_id: string | null; unit: string | null; initial_quantity: number; is_serial_tracked?: boolean | null; warehouse_id?: string | null }
...
export default function DocumentsPage() {
  ...
  const [form, setForm] = useState({
    ...
    contact_id: '', product_id: '', warehouse_id: '',
    quantity: '', document_date: new Date().toISOString().slice(0, 10),
    description: '',
    serials: '', batch_number: '', proforma_number: '', invoice_number: '',
  });
  ...
  useEffect(() => {
    ...
    if (!pid || !p?.is_serial_tracked) { setProductSerials([]); return; }
  ...
  const currentStock = useMemo(() => {
    ...
    const initial = (selectedProduct.warehouse_id ?? null) === wid ? Number(selectedProduct.initial_quantity ?? 0) : 0;
  ...
  const openCreate = () => {
    setForm({
      document_type: 'incoming', contact_id: '', product_id: '', warehouse_id: '',
      quantity: '', document_date: new Date().toISOString().slice(0, 10), description: '',
      serials: '', batch_number: '', proforma_number: '', invoice_number: '',
    });
    setSerialSearch('');
    setOpen(true);
  ...
  const save = async () => {
    if (!user) return;
    if (!form.product_id) { toast.error('انتخاب کالا الزامی است'); return; }
    ...
    if (isSerial) {
      if (serials.length === 0) { toast.error('حداقل یک سریال نامبر وارد کنید'); return; }
      const dup = serials.find((s, i) => serials.indexOf(s) !== i);
      if (dup) { toast.error(`سریال تکراری در ورودی: ${dup}`); return; }
    } else {
      if (!qty || qty <= 0) { toast.error('مقدار باید بزرگتر از صفر باشد'); return; }
    }
    if (form.document_type === 'outgoing') {
      if (currentStock <= 0) { toast.error('این کالا در انبار انتخاب شده موجود نیست'); return; }
      if (qty > currentStock) { toast.error(`موجودی انبار انتخاب شده کافی نیست. موجودی فعلی: ${formatNumber(currentStock)}`); return; }
    }
  ...
  const filtered = docs.filter((d) => {
    ...
    const cname = contactMap[d.contact_id ?? ''] ?? '';
    return matchesSearch(pname, search)
  ...
  return (
    <>
      <PageHeader title="صدور سند" description="ثبت اسناد ورود و خروج کالا"
        action={canCreate ? <Button onClick={openCreate}><Plus className="size-4" />سند جدید</Button> : null} />

      <Card className="p-4 mb-4 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute right-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
          <Input placeholder="جستجو در اسناد..." value={search} onChange={(e: any) => setSearch(e.target.value)} className="pr-10" />
        </div>
        <Select value={filterType} onChange={(e: any) => setFilterType(e.target.value as any)} className="max-w-[180px]">
      ...
      {loading ? <div className="text-center py-10 text-muted-foreground">در حال بارگذاری...</div>
        : filtered.length === 0
          ...
          : (
            <DataTable columns={['شماره سند', 'نوع سند', 'کالا', 'گروه کالا', 'طرف حساب', 'مقدار', 'واحد', 'تاریخ', 'توضیحات', 'عملیات']}>
              {filtered.map((d) => {
                const p = productMap[d.product_id ?? ''];
                return (
                  <tr key={d.id}>
                    ...
                    <td className="px-4 py-3">
                      {d.document_type === 'incoming'
                        ? <Badge tone="success"><ArrowDownToLine className="size-3 ml-1" />ورودی</Badge>
                        : <Badge tone="destructive"><ArrowUpFromLine className="size-3 ml-1" />خروجی</Badge>}
                    </td>
                    <td className="px-4 py-3 font-medium">{p?.name ?? '—'}</td>
                    <td className="px-4 py-3 text-muted-foreground">{groupMap[d.product_group_id ?? ''] ?? '—'}</td>
                    ...
                    <td className="px-4 py-3 text-muted-foreground max-w-[180px] truncate">{d.description || '—'}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        {p?.is_serial_tracked && (
                          <Button variant="ghost" onClick={() => openSerialsViewer(d)} className="px-2 py-1.5" title="نمایش سریال‌ها"><ListOrdered className="size-4" /></Button>
                        )}
                        {d.document_type === 'outgoing' && (
                          <Button variant="ghost" onClick={() => printIssueSlip(d)} className="px-2 py-1.5" title="چاپ حواله خروج"><Printer className="size-4" /></Button>
                        )}
                        {canDelete && <Button variant="ghost" onClick={() => setDel(d)} className="px-2 py-1.5 text-destructive"><Trash2 className="size-4" /></Button>}
                      </div>
                    </td>
                  </tr>
                );
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="سند انبار جدید" footer={
        <>
          <Button variant="secondary" onClick={() => setOpen(false)} disabled={busy}>انصراف</Button>
          <Button onClick={save} disabled={busy}>ثبت سند</Button>
        </>
      }>
        <div className="space-y-4">
          <Field label="نوع سند" required>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setForm({ ...form, document_type: 'incoming' })}
              ...
              <button type="button" onClick={() => setForm({ ...form, document_type: 'outgoing' })}
          ...
          <Field label="طرف حساب">
            <Select value={form.contact_id} onChange={(e: any) => setForm({ ...form, contact_id: e.target.value })}>
          ...
          <Field label="کالا" required>
            <ProductCombobox products={products} groupMap={groupMap} value={form.product_id} onChange={(id) => setForm({ ...form, product_id: id })} />
          </Field>

          <div className="grid grid-cols-2 gap-4">
            <Field label="گروه کالا (خودکار)"><Input value={autoGroup} readOnly className="bg-muted/50" /></Field>
            <Field label="واحد (خودکار)"><Input value={autoUnit} readOnly className="bg-muted/50" /></Field>
          </div>

          {selectedProduct && (
            <div className={`text-xs px-3 py-2 rounded-lg ${form.document_type === 'outgoing' && Number(form.quantity) > currentStock ? 'bg-destructive/15 text-destructive' : 'bg-muted text-muted-foreground'}`}>
              موجودی فعلی: <strong>{formatNumber(currentStock)} {autoUnit}</strong>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="مقدار" required>
              <Input type="number" step="0.01" dir="ltr"
                value={selectedProduct?.is_serial_tracked ? String(parseSerials(form.serials).length) : form.quantity}
              ...
              {selectedProduct?.is_serial_tracked && <span className="text-[11px] text-muted-foreground">مقدار از تعداد سریال‌ها محاسبه می‌شود</span>}
            </Field>
            <Field label="تاریخ سند" required>
              <Input type="date" dir="ltr" value={form.document_date} onChange={(e: any) => setForm({ ...form, document_date: e.target.value })} />
              {form.document_date && <span className="text-[11px] text-muted-foreground">معادل شمسی: {formatJalali(form.document_date)}</span>}
            </Field>
          </div>
          ...
          {selectedProduct?.is_serial_tracked && (
            <div className="border-t pt-3 space-y-3 bg-primary/5 -mx-1 px-3 py-3 rounded-lg">
              <div className="text-sm font-bold flex items-center gap-2">
                ...
                {form.document_type === 'incoming' ? 'ثبت سریال نامبرهای ورودی' : 'انتخاب سریال نامبرهای خروج'}
              </div>
                ...
                <>
                  <Field label="سریال نامبرها" required>
                    <Textarea
                      ...
                      placeholder={'هر سریال در یک خط، یا با کاما جدا کنید\nمثال:\nSN-001\nSN-002\nSN-003'} />
                    <span className="text-[11px] text-muted-foreground">
                      تعداد وارد شده: {toFaDigits(parseSerials(form.serials).length)}
                    </span>
                  </Field>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <Field label="شماره بچ"><Input value={form.batch_number} onChange={(e: any) => setForm({ ...form, batch_number: e.target.value })} /></Field>
                    <Field label="شماره پروفرما"><Input value={form.proforma_number} onChange={(e: any) => setForm({ ...form, proforma_number: e.target.value })} /></Field>
                    <Field label="شماره اینویس"><Input value={form.invoice_number} onChange={(e: any) => setForm({ ...form, invoice_number: e.target.value })} /></Field>
                  </div>
                </>
              )}
              ...
              {form.document_type === 'outgoing' && (() => {
                ...
                if (available.length === 0) {
                  return <div className="text-xs text-destructive">هیچ سریال در دسترسی برای این کالا یافت نشد.</div>;
                }

                const allVisibleSelected = visible.length > 0 && visible.every((s: any) => selected.has(s.serial_number));
                const toggleAllVisible = () => {
                  const next = new Set(selected);
                  if (allVisibleSelected) visible.forEach((s: any) => next.delete(s.serial_number));
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <div className="relative flex-1">
                        ...
                        <Input value={serialSearch} onChange={(e: any) => setSerialSearch(e.target.value)}
                          placeholder="جستجو در سریال، بچ، پروفرما، اینویس..." className="pr-9 h-9 text-xs" />
                      </div>
                      <Badge tone="default">انتخاب شده: {toFaDigits(selected.size)} / {toFaDigits(available.length)}</Badge>
                    </div>
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {visible.map((s: any) => {
                            ...
                            return (
                              <tr key={s.id} onClick={() => toggleOutgoingSerial(s.serial_number)}
                                className={`border-t cursor-pointer hover:bg-primary/10 ${isSel ? 'bg-primary/5' : ''}`}>
                                <td className="px-2 py-1.5"><input type="checkbox" checked={isSel} readOnly /></td>
                                ...
                                <td className="px-2 py-1.5"><Badge tone="success">آماده</Badge></td>
                              </tr>
                            );
                          })}
                          {visible.length === 0 && (
                            <tr><td colSpan={7} className="px-2 py-4 text-center text-muted-foreground">نتیجه‌ای مطابق جستجو یافت نشد</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
                    <div className="text-[11px] text-muted-foreground">مقدار سند به‌صورت خودکار برابر تعداد سریال‌های انتخاب‌شده محاسبه می‌شود.</div>
                  </div>
                );
              })()}
            </div>
      ...
      <Modal open={!!viewSerialsDoc} onClose={() => { setViewSerialsDoc(null); setViewSerialsList([]); }}
        title={viewSerialsDoc ? `سریال‌های سند شماره ${toFaDigits(viewSerialsDoc.doc_number)}` : 'سریال‌ها'}>
        {viewSerialsDoc && (
          <div className="space-y-3">
            <div className="text-xs text-muted-foreground">
              {productMap[viewSerialsDoc.product_id ?? '']?.name ?? '—'} — تعداد: {toFaDigits(viewSerialsList.length)}
            </div>
                  ...
                  {viewSerialsList.map((s, i) => (
                    <tr key={s.id} className="border-t">
                      ...
                      <td className="px-2 py-1.5">
                        {s.status === 'available' ? <Badge tone="success">آماده</Badge> : <Badge tone="destructive">خروج‌شده</Badge>}
                      </td>
                    </tr>
                  ))}
                  {viewSerialsList.length === 0 && (
                    <tr><td colSpan={7} className="px-2 py-6 text-center text-muted-foreground">سریالی برای این سند ثبت نشده است</td></tr>
                  )}
                </tbody>
              </table>
            </div>
