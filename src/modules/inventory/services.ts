...
export const inventoryService = {
  ...
  async create(payload: any) {
    const { data, error } = await supabase.from('inventory_documents').insert(payload).select().single();
    if (error) throw error;
    return data;
  },
 ...
 */
export function calculateStock(docs: Array<{ product_id: string | null; document_type: string; quantity: number }>) {
